...
create or replace function pgr_aStarTPP_radial_search_closest_sorting_1_via(IN tbl character varying,
...
---------------------
	...
	For i in 1..breakwhile Loop
		execute  'with distance as (
			select id, x, y, geom from pgr_aStarTPP_radial_search_loop_nodes_rubbish, pgr_aStarTPP_stdistance_closest_route where id = '||$6[i]||' order by st_distance(geom_route, geom) limit 1
			)
			insert into pgr_aStarTPP_radial_search_loop_nodes (id, x, y, geom) select id, x, y, geom from distance limit 1;';
	end loop;
---------------------
	...
	sql_tsp := 'select id as id2 from pgr_aStarTPP_radial_search_matrix order by id';
	seq := 0;
	-- We have declared source_var initial value as -1, not 0, coz any positive number could be the node_id of a point in ways_vertices_pgr table. --
	-- But by making it negative, we are assuring that the following loop will enter only at the first time in the "If" section and no more later. --
	source_var := -1;
...
-- select geom from pgr_aStarTPP_radial_search_closest_sorting_1_via('ways', 29.104768000000000,41.027425000000001,29.122485000000001,41.048262999999999, 101)
