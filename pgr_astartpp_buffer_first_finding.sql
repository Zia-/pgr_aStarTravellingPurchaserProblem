...
create or replace function pgr_aStarTPP_buffer_first_finding(IN tbl character varying, 
...
begin
	...
	-- We are defining this ending_id coz coordinates must be entered into the matrix table (for pgr_tsp) in the order: --
	...
	-- Calculate the via point details and insert them into the matrix table using the "SELECT COALESCE" METHOD --
	...
	For i in 1..breakwhile Loop
		...
		execute  'with buffer as (
			select coalesce(
			(select st_makepoint(x, y) from individual_stops, route where st_dwithin(the_geom, geom_route, 0.00025) and id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1),
			(select st_makepoint(x, y) from individual_stops, route where st_dwithin(the_geom, geom_route, 0.00050) and id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1),
			(select st_makepoint(x, y) from individual_stops, route where st_dwithin(the_geom, geom_route, 0.001) and id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1),
			(select st_makepoint(x, y) from individual_stops, route where st_dwithin(the_geom, geom_route, 0.002) and id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1),
			(select st_makepoint(x, y) from individual_stops, route where st_dwithin(the_geom, geom_route, 0.004) and id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1),
			(select st_makepoint(x, y) from individual_stops, route where st_dwithin(the_geom, geom_route, 0.008) and id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1),
			(select st_makepoint(x, y) from individual_stops, route where st_dwithin(the_geom, geom_route, 0.02) and id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1),
			(select st_makepoint(x, y) from individual_stops, route where st_dwithin(the_geom, geom_route, 0.04) and id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1),
			(select st_makepoint(x, y) from individual_stops, route where st_dwithin(the_geom, geom_route, 0.08) and id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1),
			(select st_makepoint(x, y) from individual_stops, route where st_dwithin(the_geom, geom_route, 0.2) and id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1)
			) 
			as geom_buffer)
		...
		*/
		execute  'with buffer as (
			select st_makepoint(x, y) as geom_buffer from individual_stops, route where id = '||$6[i]||' order by st_distance(geom_route, the_geom) limit 1 
			)
			insert into matrix (id, node_id, x, y) select '||via_id||', id, st_x(the_geom)::double precision, st_y(the_geom)::double precision
			from ways_vertices_pgr, buffer ORDER BY the_geom <-> st_setsrid(geom_buffer, 4326) limit 1;'; 
	end loop;
	-- Calculate the TSP from the above created matrix table (from here onwards I am using the pgr_aStarFromAtoBviaC() approach) --
	-- Better code will use pgr_aStarFromAtoBviaC() direcltly, instead of writing the same stuff again here --
	...
	-- By making it negative, we are assuring that the following loop will enter only at the first time in the "If" section and no more later. --
	source_var := -1;
	...
	FOR rec_tsp IN EXECUTE sql_tsp
		LOOP
			...
			Else
				...
				-- Extracting the geom obtained from each pair aStar() in a row by row manner and returning it back to the pgr_aStarTPP_buffer_first_finding() --
...
-- select geom from pgr_aStarTPP_buffer_first_finding('ways', 28.97438,41.00311,28.95370,41.02163, 101, 102, 103)
