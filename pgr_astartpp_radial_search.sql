...
begin
	...
	-- loop_nodes table will store the cloest A, B, and C nodes wrt to the start and end points -- 
	create temporary table loop_nodes (id integer, x double precision, y double precision, geom geometry);
	-- Feed the loop_nodes table with those A, B, and C nodes which are cloest to the start point --
	For i in 1..breakwhile
		...
		End Loop;
	-- Feed the loop_nodes table with those A, B, and C nodes which are cloest to the end point --
	For i in 1..breakwhile
		Loop
		...
		End Loop;
	-- This matrix table will store the shortest route combination of A, B, and C nodes --
	create temporary table matrix (id integer, node_id integer, x double precision, y double precision);
	-- This matrix_sub table will store all possible combinations of A, B, and C nodes, coming from the loop_nodes table --
					...
					FOR rec_loop3 IN EXECUTE sql_loop3
						Loop
							...
							If (sum_cost_old = 0) Then
								-- Feeding matrix table for the first time --
							...
							Else
								...
								If (sum_cost < sum_cost_old) then
									-- Delete all rows from matrix table --
									delete from matrix;
									-- Insert this new matrix combination which is even more smaller than our smallest route nodes combination --
									insert into matrix (id, node_id, x, y) select id, node_id, x, y from matrix_sub;
									-- Making sum_cost_old value our sum_cost value --
									sum_cost_old := sum_cost;
								Else
								END IF;
							END IF;
							-- Empty matrix_sub table to handle another A, B, and C combination --
	...
	FOR rec_tsp IN EXECUTE sql_tsp
		LOOP
			...
			END IF;
		END LOOP;	
	-- Drop the temporary tables, otherwise the next time you will run the query it will show that the matrix, loops_nodes or matrix_sub table already exists --
